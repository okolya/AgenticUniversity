...
Subagent context is isolated. Route into the `university/` Git repository first, then read `AGENTS.md`, `university/professions/rector/PROFESSION.md`, `university/professions/rector/SKILLS.md`, and `university/protocols/worker-activation.md` relative to that repository. Follow `university/policies/runtime-command-whitelist.md` for the bounded read-only bootstrap surface.
